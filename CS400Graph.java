...
public class CS400Graph<NodeType,EdgeType extends Number> implements GraphADT<NodeType,EdgeType> {
    ...
    protected Path dijkstrasShortestPath(NodeType start, NodeType end) {
        ...
        //Step 1: always add the first starting vertex to visited list.
        Vertex startingV = vertices.get(start);
        visited.add(startingV);

        //Step 2: add nodes from starting node to the successor of pq.
        for(int i = 0; i < startingV.edgesLeaving.size(); i++){
            Path startP = new Path(new Path(startingV), startingV.edgesLeaving.get(i));
            pq.add(startP);
        }

        //Step 3: Remove the shortest path when pq is not empty.
        while(!pq.isEmpty()) {
            Path shortestP = pq.remove();
            Vertex visitingV = shortestP.end; //the vertex were visiting.
            visited.add(visitingV);
            Vertex endingV = vertices.get(end);
            if (visitingV.equals(endingV)) {return shortestP;}
            //Check if the successor vertex is not in the visited list
            // to extend this vertex with the current path and add that path to pq.
            for (int i = 0; i < visitingV.edgesLeaving.size(); i++) {
                if (!visited.contains(visitingV.edgesLeaving.get(i).target)) {
                    pq.add(new Path(shortestP, visitingV.edgesLeaving.get(i)));
                }
            }
        }
        //Else, the search fails to find any path from the start to end vertices.
